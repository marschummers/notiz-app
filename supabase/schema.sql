...
-- Aktueller Projektstand und bewusst dokumentierter Entscheidungsverlauf je Themenbereich.
...
create index if not exists notiz_project_section_documents_project_idx on notiz_project_section_documents (project_id);
create index if not exists notiz_project_section_document_revisions_project_idx on notiz_project_section_document_revisions (project_id, created_at desc);
create index if not exists notiz_project_section_document_revisions_section_idx on notiz_project_section_document_revisions (section_id, created_at desc);
...
create policy "project_section_documents_team" on notiz_project_section_documents
  for all to authenticated
  using (notiz_is_approved() and notiz_can_access_project(project_id))
  with check (notiz_is_approved() and auth.uid() = user_id and auth.uid() = updated_by_user_id and notiz_can_access_project(project_id));
...
create policy "project_section_document_revisions_team" on notiz_project_section_document_revisions
  for all to authenticated
  using (notiz_is_approved() and notiz_can_access_project(project_id))
  with check (notiz_is_approved() and auth.uid() = user_id and auth.uid() = changed_by_user_id and notiz_can_access_project(project_id));

do $$
declare table_name text;
begin
  foreach table_name in array array['notiz_project_section_documents','notiz_project_section_document_revisions']
  loop
...
create policy "project_task_comments_owner_update" on notiz_project_task_comments
  ...
  using (notiz_is_approved() and exists (
    select 1 from notiz_project_tasks task join notiz_projects project on project.id = task.project_id
    where task.id = task_id and task.deleted_at is null and project.deleted_at is null and project.owner_user_id = auth.uid()
  ))
  with check (notiz_is_approved() and exists (
    select 1 from notiz_project_tasks task join notiz_projects project on project.id = task.project_id
    where task.id = task_id and task.deleted_at is null and project.deleted_at is null and project.owner_user_id = auth.uid()
  ));
