...
export default function ProjectSidebar({ userId, navigation, onNavigate }: { userId: string; navigation: ProjectNavigation; onNavigate: (navigation: ProjectNavigation) => void }) {
  ...
  const results = query ? projects.filter((project) => `${project.name} ${project.customerName ?? ''}`.toLocaleLowerCase('de').includes(query)).slice(0, 8) : []
  ...
  return <div className="project-sidebar-content">
    <div className="sidebar-section project-sidebar-tools">
      ...
      {query && <div className="project-sidebar-results">{results.length ? results.map((project) => <SidebarLink key={project.id} label={project.name} detail={project.customerName} active={navigation.type === 'project' && navigation.id === project.id} onClick={() => onNavigate({ type: 'project', id: project.id })}/>) : <p className="sidebar-hint">Keine Projekte gefunden.</p>}</div>}
    ...
    <SidebarGroup title="Meine Projekte" className="project-sidebar-scroll">
      {mine.length ? mine.map((project) => <SidebarLink key={project.id} label={project.name} detail={project.customerName} active={navigation.type === 'project' && navigation.id === project.id} onClick={() => onNavigate({ type: 'project', id: project.id })}/>) : <p className="sidebar-hint">Keine aktiven Projekte.</p>}
    </SidebarGroup>
